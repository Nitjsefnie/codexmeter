...
def parse_file(file_key: str, blob: bytes) -> dict:
    ...
    Auto-detects legacy kimi-cli, kimi-code and Codex rollout format per
    file, and raises UnsupportedTranscriptError for a Claude Code
    transcript — which reaches this bucket whenever claude-code-proxy
    routes a Claude Code session through Codex.
    records: list of dicts with keys
      file_key, line_num, uuid, ts, model,
      fresh_tokens, cache_creation_tokens, cache_read_tokens,
      output_tokens, text_chars, reply_latency_s, cost_usd
    ...
    for raw in blob.splitlines():
        ...
        if "timestamp" in obj and (obj.get("message") or {}).get("type") in {
            ...
            fmt = "legacy"
            break
        # Last rung on purpose: a line that somehow carried both a Codex or
        # Kimi marker and a sessionId belongs to the format we can parse.
