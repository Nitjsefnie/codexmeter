...
- The completing implementation commit includes `Closes #7`.
...
Keep the current value reader (`finiteSeriesValue`) and event ordering. Use each
bounded `bin.end` for cumulative points and tooltip text. In the SVG bar loop,
replace the global `barW` with:
...
```bash
pytest -q tests/test_time_series_geometry.py tests/test_dashboard_bin_size.py tests/test_dashboard_token_contract.py
...
Expected: 342 or more pytest tests pass; every lint/type command exits 0.

- [ ] **Step 2: Audit every new commit trailer**

Verify every commit after `origin/master` contains the exact co-author trailer
and the completing implementation commit contains `Closes #7`.
...
- [ ] **Step 4: Restart only the existing codexmeter service**

Resolve the installed unit with:

```bash
systemctl list-unit-files --type=service | rg -i 'codexmeter'
```

Restart the exact matching unit, wait until `systemctl is-active` reports
`active`, then require `curl -fsS https://codex.nitjsefni.eu/` to succeed.
...
complete successfully and confirm issue #7 is closed.
