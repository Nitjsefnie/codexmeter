    ...
    with _pg_cur() as cur:
        cur.execute(
            ...
            (json.dumps([
                ...
                # Computed RELATIVE TO NOW: pinned to an absolute date it
                # silently stops being inside `range=30d` once the calendar
                # passes it, and the test then fails for a reason that has
                # nothing to do with what it checks. That is exactly what
                # happened — it was pinned to 2026-07-20 and began failing
                # on 2026-08-19.
