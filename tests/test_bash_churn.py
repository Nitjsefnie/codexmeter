...
# --- argv arrays (Codex `monitor`, and any API taking command: [...]) ------

def test_argv_shell_dash_c_payload_is_counted():
    """`monitor` takes command:[argv], not a cmd string. A shell -c
    element is a whole script — count it as one."""
    argv = ["bash", "-lc", "cat > f <<'EOF'\na\nb\nEOF\n"]
    assert argv_churn(argv) == (2, 0)


def test_argv_sh_dash_c_payload_is_counted():
    argv = ["sh", "-c", "cat >> log <<'EOF'\nx\nEOF\n"]
    assert argv_churn(argv) == (1, 0)


def test_argv_python_dash_c_payload_is_counted():
    argv = ["python3", "-u", "-c",
            "import pathlib\npathlib.Path('f').write_text('a\\nb\\n')"]
    assert argv_churn(argv) == (2, 0)


def test_argv_without_an_inline_script_is_zero():
    """A plain program invocation carries no payload to count, and its
    own arguments must not be misread as one."""
    assert argv_churn(["python3", "tests/run_tests.py"]) == (0, 0)
    assert argv_churn(["gh", "run", "watch", "31975363393"]) == (0, 0)


def test_argv_watcher_script_that_writes_nothing_is_zero():
    """The common monitor payload is a poll loop printing to stdout."""
    argv = ["python3", "-u", "-c",
            "import shutil\nwhile True:\n    print(shutil.disk_usage('/').free)"]
    assert argv_churn(argv) == (0, 0)


def test_argv_that_is_not_a_list_of_strings_is_zero():
    assert argv_churn([]) == (0, 0)
    assert argv_churn(["bash", "-lc", None]) == (0, 0)
