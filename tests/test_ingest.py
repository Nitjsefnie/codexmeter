...
# --- Claude Code transcripts in the Codex bucket -----------------------------
#
# claude-code-proxy lets a Claude Code session run against Codex.
# archive_sessions.py routes that session's transcript to the Codex bucket by
...
def test_claude_transcript_in_claude_layout_is_skipped(fresh_db, mini_r2_env):
    ...
    _put_claude(mini_r2_env, "-root-codexmeter/ses-1/data/subagents/a1.jsonl")
    result = ingest.run_ingest(trigger="manual")
    assert result["error"] is None
    assert result["skipped"] == 2
    # The Codex files still ingest normally, and nothing Claude-shaped lands.
    assert result["inserted"] == 5
    with db.viz_conn() as c:
...
def test_claude_transcript_under_sessions_is_skipped_not_marked_parsed(
    ...
    native layout, so the parser is the only thing standing between them and
